...
function createEl(el, className) {
  let newDiv = document.createElement(el);
  newDiv.className = className;
  return newDiv;
}

function goodsToHTML(arr) {
  let goodsCont = document.getElementById("goods-container");
  let row = createEl("div", "row");
  goodsCont.appendChild(row);

  for (i = 0; i < arr.length; i++) {
    let col = createEl("div", "col-4 stretch");
    let item = createEl("div", "item");
    let itemImg = createEl("div", "item-img");
    let img = createEl("img", "image");
    img.src = "img/img-placeholder.png";
    itemImg.appendChild(img);
    item.appendChild(itemImg);
    row.appendChild(col);
    col.appendChild(item);
    let itemName = createEl("div", "item-name");
    let itemNameLink = createEl("a", "item-name-link");
    let name = document.createTextNode(arr[i].name);
    itemNameLink.appendChild(name);
    itemName.appendChild(itemNameLink);
    item.appendChild(itemName);
    let reviewsHolder = createEl("div", "item-reviews-holder");
    let reviewsText = createEl("span", " item-reviews-text");
    let reviews = document.createTextNode(arr[i].ratingRevievs);
    reviewsText.appendChild(reviews);
    reviewsHolder.appendChild(reviewsText);
    item.appendChild(reviewsHolder);
    let priceHolder = createEl("div", "item-price-holder");
    if (typeof (arr[i].price) === "object") {
      let oldPrice = createEl("span", "item-price__old");
      let oldPriceText = document.createTextNode(arr[i].price.oldUan);
      oldPrice.appendChild(oldPriceText);
      priceHolder.appendChild(oldPrice);
      let newPrice = createEl("span", "item-price__new");
      let newPriceText = document.createTextNode(arr[i].price.newUan);
      newPrice.appendChild(newPriceText);
      priceHolder.appendChild(newPrice);
      item.appendChild(priceHolder);
    } else {
      let defPrice = createEl("span", "item-price");
      let defPriceText = document.createTextNode(arr[i].price);
      defPrice.appendChild(defPriceText);
      priceHolder.appendChild(defPrice);
      item.appendChild(priceHolder);
...
function removeGoodsItems() {
  let goodsContainer = document.getElementById("goods-container");
  while (goodsContainer.firstChild) {
    goodsContainer.firstChild.remove();
  }
}

goodsToHTML(arrItems);

document.getElementById("sorting").onchange = function () {
  switch (this.value) {
    case "priceFromLow":
      removeGoodsItems();
      arrItems.sort(comparePriceFromLow);
      goodsToHTML(arrItems);
      break;
    case "priceFromHigh":
      removeGoodsItems();
      arrItems.sort(comparePriceFromHigh);
      goodsToHTML(arrItems);
      break;
    case "reviewsFromLow":
      removeGoodsItems();
      arrItems.sort(compareReviewFromLow);
      goodsToHTML(arrItems);
      break;
    case "reviewsFromHigh":
      removeGoodsItems();
      arrItems.sort(compareReviewFromHigh);
      goodsToHTML(arrItems);
      break;
  }
}
